...
This is a DeepSurv loss based net for RFS prediction of OPC cancer, input can be only image (CT/PET) and/or real GTV
...
from utlis import make_surv_array , get_breaks , surv_likelihood,  get_surv_time, get_data_dict, get_model, plot_images
...
def main():
    ...
    opt.result_path = opt.result_path + str(opt.model)  + ' '+ str(opt.event_name) +'_lr' + str(opt.learning_rate)+ '_' + opt.optimizer + '_break(d)_' + str(opt.breaks_interval) + '_bs' + str(opt.batch_size) + '_input_' + str(opt.input_modality)+ '_intype_' + opt.input_type + '_OS_' + str(opt.oversample) + '_pretrain_' + str(opt.pretrain) + '_fold' + str(opt.fold) + '_MDA5'
