...
This is a DeepSurv loss based net for RFS prediction of OPC cancer, input can be only image (CT/PET) and/or GTV(autoseg)
...
from utlis import make_surv_array , get_breaks , surv_likelihood,  get_surv_time, get_data_dict_trainusingHungSeg, get_model, plot_images
import torch.nn as nn
from para_opts import parse_opts
from torch.optim.lr_scheduler import MultiStepLR
import wandb
from test import test_epoch, test_epoch_tumor
import losses
...
def main():
    ...
    train_data_dict = get_data_dict_trainusingHungSeg(train_ID, opt )    # juest for test
    val_data_dict = get_data_dict_trainusingHungSeg(val_ID, opt )  
    test_data_dict = get_data_dict_trainusingHungSeg(test_ID, opt)  
    ...
    train_transforms = Compose([LoadImaged(keys = [image for image in opt.input_modality]),
                                ...
                                Resized(keys = ['CT','PT','PT_nobrain','gtv','clinical','radiomics'], spatial_size = (96, 96 ,96),  mode = ('trilinear', 'trilinear','trilinear','nearest','nearest','nearest'), align_corners= (True,True,True,None,None,None),  allow_missing_keys = True) , 
    ...
    if not opt.no_train:
        ...
        for epoch in range(opt.begin_epoch, max_epochs):
            ...
            for batch_data in train_loader:
                ...
                print(f"{step}/{epoch_len}, train_loss: {loss.item():.4f},neglike_loss: {neglog_loss:.4f},l2_norm: {l2_norm:.4f} ")
            ...
            test_epoch_tumor(model, train_loader_test, device, opt, endpoint_info, train_ID, mode ='train')
            ...
            if not opt.no_val:
                if epoch % val_interval == 0:
                    ...
                    test_epoch_tumor(model, test_loader, device, opt, endpoint_info, test_ID, mode ='test')
    ...
    if not opt.no_test:
        ...
        # give yan my risk output
        all_ID = list(endpoint_info['PatientID'])  # Use val as test

        all_data_dict = get_data_dict_trainusingHungSeg(all_ID, opt )    # juest for test    
    
        all_ds = Dataset(data=all_data_dict, transform=val_transforms)
        all_loader = DataLoader(all_ds, batch_size=opt.batch_size, num_workers=num_workers, pin_memory=pin_memory)
    
        test_outputs_cat = torch.Tensor().to(device)
        with torch.no_grad():
            for test_data in all_loader:
            ...
            epoch_time =  list(endpoint_info.loc[endpoint_info['PatientID'].isin(all_ID)][opt.event_time_name])
            epoch_event = list(endpoint_info.loc[endpoint_info['PatientID'].isin(all_ID)][opt.event_name])
            
            metric = concordance_index(epoch_time, epoch_score, epoch_event) # test_cindex
            print ('Final all C-index: ', metric)
            risk_out =pd.DataFrame()
            risk_out['PatientID'] = endpoint_info['PatientID']
            risk_out['risk_fold'+ str(opt.fold)] = test_outputs_cat
            risk_out.to_csv('/data/pg-umcg_mii/data/hecktor2022/outpred/DeepSurv/PT_nobrain--HungSeg_risk_fold' +str(opt.fold) + '.csv' )
